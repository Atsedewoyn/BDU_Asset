        ...
        <v-container grid-list-xs>
            <v-row justify="center">
                <v-card class="text-center mx-15 my-5 px-15" width="600">
                    ...
                    <v-card-text>
                        <v-text-field name="full name" label="full name" v-model="name" :rules="nameRules" outlined>
                        </v-text-field>
                        <v-text-field name="axion type" label="axion type" v-model="axion_type" :rules="axion_typeRules"
                            outlined required>
                        </v-text-field>
                        <v-text-field name="axion amount" label="insert axion amount you want to buy"
                            v-model="axion_amount" :rules="axion_amountRules" outlined required>
                        </v-text-field>
...
<script>
import mutationFavorite from '~/apollo/mutations/request_mutations'
export default {
    layout: 'blank',
    data() {
        return {
    ...
    methods: {
        insertOrders() {
            this.$apollo.mutate({
                mutation: mutationFavorite.insert_favorites,
                variables: {
                    name: this.name,
                ...
                this.favorites.push(rs.data.insert_favorites)
                this.$swal({
                    toast: true,
